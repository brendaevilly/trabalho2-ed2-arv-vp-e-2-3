...
void mostrarArtistasPorEstilo(Arvore *artistas, char *estilo) {
    if (artistas){
        ...
        if(compInfo1 == 0){
            ...
            if(compInfo1 < 0)
                nInfoBuscada = buscarNaArvore23(artistas->esq, estilo, busca, R);
            else if(artistas->nInfos == 1 || (compInfo2 != 2 && compInfo2 < 0))
                nInfoBuscada = buscarNaArvore23(artistas->cen, nome, busca, R);
            else nInfoBuscada = buscarNaArvore23(artistas->dir, nome, busca, R);
        }
    }
}
